	...
	do {
		...
		if (is_new) { // insert into account table
			if (SQL_SUCCESS != stmt.Prepare(
				...
				SQL_SUCCESS !=
					stmt.BindParam(8, SQLDT_INT32, (void*)&acc->expiration_time, sizeof(acc->expiration_time)) ||
...
bool account_db_sql_enable_monitor_vip(AccountDB* self, const uint32 account_id, time_t vip_time) {
	...
	ld->vip_timeout_tid = add_timer(
		gettick() + remaining * 1000, account_vip_timeout_timer, account_id, reinterpret_cast<intptr_t>(self));
