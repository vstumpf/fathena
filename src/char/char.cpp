...
int32 char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p) {
	...
	if ((p->hair != cp->hair) || (p->hair_color != cp->hair_color) || (p->clothes_color != cp->clothes_color) ||
		...
		(p->disable_partyinvite != cp->disable_partyinvite) || (p->disable_showcostumes != cp->disable_showcostumes)) {
		if (SQL_ERROR ==
...
bool char_memitemdata_from_sql(struct s_storage* p, int32 max, int32 id, enum storage_type tableswitch, uint8 stor_id) {
	...
	if (SQL_ERROR == stmt.PrepareStr(StringBuf_Value(&buf)) || SQL_ERROR == stmt.BindParam(0, SQLDT_INT32, &id, 0) ||
...
int32 char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count) {
	...
	if (SQL_ERROR ==
			stmt.Prepare(
				...
				"`clothes_color`,`body`,`weapon`,`shield`,`head_top`,`head_mid`,`head_bottom`,`last_map`,`rename`,`"
				"delete_date`,"
				"`robe`,`moves`,`unban_time`,`font`,`uniqueitem_counter`,`sex`,`hotkey_rowshift`,`title_id`,`show_"
				"equip`,"
				"`hotkey_rowshift2`,"
				...
				sd->account_id,
				MAX_CHARS) ||
		SQL_ERROR == stmt.Execute() || SQL_ERROR == stmt.BindColumn(0, SQLDT_INT32, &p.char_id) ||
		...
		SQL_ERROR == stmt.BindColumn(8, SQLDT_INT32, &p.zeny) || SQL_ERROR == stmt.BindColumn(9, SQLDT_INT16, &p.str) ||
...
int32 char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if (SQL_ERROR ==
			stmt.Prepare(
				...
				"hotkey_rowshift2`,"
				"`max_ap`,`ap`,`trait_point`,`pow`,`sta`,`wis`,`spl`,`con`,`crt`,"
				"`inventory_slots`,`body_direction`,`disable_call`,`last_instanceid`,`disable_partyinvite`,`disable_"
				"showcostumes`"
				" FROM `%s` WHERE `char_id`=? LIMIT 1",
				schema_config.char_db) ||
		SQL_ERROR == stmt.BindParam(0, SQLDT_INT32, &char_id, 0) || SQL_ERROR == stmt.Execute() ||
