...
bool sv_readdb(const char* directory,
	...
	while (fgets(line, static_cast<int32>(maxcols * colsize), fp)) {
		...
		if (columns < mincols) {
			ShowError("sv_readdb: Insufficient columns in line %d of \"%s\" (found %" PRIuPTR
					  ", need at least %" PRIuPTR ").\n",
					  lines,
					  path,
					  columns,
					  mincols);
		...
		if (entries == maxrows) {
			ShowError("sv_readdb: Reached the maximum allowed number of entries (%" PRIuPTR
					  ") when parsing file \"%s\".\n",
					  maxrows,
					  path);
			break;
	...
	fclose(fp);
	ShowStatus(
		"Done reading '" CL_WHITE "%" PRIuPTR CL_RESET "' entries in '" CL_WHITE "%s" CL_RESET "'.\n", entries, path);
