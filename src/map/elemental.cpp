	...
	if (!battle_check_range(ed, bl, skill_get_range(skill_id, skill_lv))) {
		...
		if (!unit_walktobl(ed, bl, skill_get_range(skill_id, skill_lv), 2)) {
			...
			if (skill_get_inf(skill_id) & INF_GROUND_SKILL) {
				ed->ud.skilltimer =
					add_timer(tick + (t_tick)status_get_speed(ed) * walk_dist, skill_castend_pos, ed->id, 0);
			}
			else {
				ed->ud.skilltimer =
					add_timer(tick + (t_tick)status_get_speed(ed) * walk_dist, skill_castend_id, ed->id, 0);
			}
		}
		return 1;
	}
...
static int32 elemental_ai_sub_timer_activesearch(block_list *bl, va_list ap) {
	...
	if (((*target) == nullptr || !check_distance_bl(ed, *target, dist)) &&
		battle_check_range(ed, bl, ed->db->range2)) { // Pick closest target?
		(*target) = bl;
		ed->target_id = bl->id;
	...
	if (mode == EL_MODE_AGGRESSIVE) {
		...
		if (!target) {
			map_foreachinallrange(
				elemental_ai_sub_timer_activesearch, ed, view_range, BL_CHAR, ed, &target, status_get_mode(ed));
		}
