...
extern int16 current_equip_item_index; // for New CARDS Scripts. It contains Inventory Index of the EQUIP_SCRIPT caller
									   // item. [Lupus]
...
void script_attach_state(struct script_state *st) {
	...
	if (st->rid && (sd = map_id2sd(st->rid)) != nullptr) {
		...
		if (sd->npc_idle_timer == INVALID_TIMER && !sd->state.ignoretimeout) {
			sd->npc_idle_timer =
				add_timer(gettick() + (SECURE_NPCTIMEOUT_INTERVAL * 1000), npc_secure_timeout_timer, sd->id, 0);
		}
	...
	for (pl_sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC *)mapit_next(iter)) {
		...
		switch (type) {
			...
			case 2: // SavePoint
				...
				break;
			case 3: // m,x,y
				if (!map_getmapflag(pl_sd->m, MF_NORETURN) && !map_getmapflag(pl_sd->m, MF_NOWARP) &&
					pc_job_can_entermap((enum e_job)pl_sd->status.class_, m, pc_get_group_level(pl_sd))) {
	...
	switch (type) {
		...
		case BG_INFO_MAPS: {
			...
			for (i = 0; i < bg->maps.size(); i++) {
				setd_sub_str(
					st, nullptr, ".@bgmaps$", static_cast<int32>(i), mapindex_id2name(bg->maps[i].mapindex), nullptr);
			}
...
BUILDIN_FUNC(consumeitem) {
	...
	// Set the item id to the item id of the script that will be executed (needed for announcement of group containers
	// for example)
