...
static bool status_change_start_post_delay(block_list *src,
										   block_list *bl,
										   sc_type type,
										   int32 val1,
										   int32 val2,
	...
	if (!(flag & 1)) {
		if (sc) {
			...
			for (const auto &it : status_db) {
				...
				// For non-players, Wink Charm, Voice of Siren and Deep Sleep end only when damage was dealt (e.g. Wink
				// Charm does not end itself) For players, these status changes end even if no damage was dealt (e.g.
				// Provoke ends them on players but not on monsters) Other status changes end even on 0 damage (e.g.
				// Wink Charm ends Freeze)
				if ((type == SC_WINKCHARM || type == SC_VOICEOFSIREN || type == SC_DEEPSLEEP) &&
					target->type != BL_PC && hp == 0) {
					continue;
				}
...
bool status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int32 flag) {
	...
	if (sc != nullptr && !sc->empty()) {
		...
		// Only do skill specific checks here, normal attacks should have been checked already (additionally the skill
		// db lookup would fail)
		...
		if (sc->option) {
			// We do not check it for non-players here as hide will not stop monsters from scanning for new targets and
			// use skills The logic that normal attacks will not actually be executed when hidden needs to be put in the
			// AI code instead
...
int32 status_base_amotion_pc(map_session_data *sd, struct status_data *status) {
	...
	aspd = ((int32)(temp_aspd + ((float)(status_calc_aspd(sd, &sd->sc, true) + val) * status->agi / 200)) -
			min(aspd, 200));
	return aspd;
#else
	if (job == nullptr) {
...
int32 status_calc_pc_sub(map_session_data *sd, uint8 opt) {
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory_data[index]->type == IT_WEAPON) {
			...
			if (sd->inventory_data[index]->script && (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) ||
													  !itemdb_isNoEquip(sd->inventory_data[index], sd->m))) {
		...
		if (sd->inventory_data[index]) {
			...
			for (j = 0; j < MAX_SLOTS; j++) { // Uses MAX_SLOTS to support Soul Bound system [Inkfish]
				...
				if (opt & SCO_FIRST && data->equip_script &&
					(pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) ||
					 !itemdb_isNoEquip(data.get(), sd->m))) { // Execute equip-script on login
				...
				if (!pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) &&
					itemdb_isNoEquip(data.get(), sd->m)) { // Card restriction checks.
					continue;
				}
				if (i == EQI_HAND_L &&
	...
	if (!sc->empty()) {
		...
		for (const auto &it : *sc) {
			if (std::shared_ptr<s_status_change_db> scdb = status_db.find(it.first);
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, status_change *sc) {
	...
	// No natural SP regen
	if (sc->getSCE(SC_DANCING) || sc->getSCE(SC_MAXIMIZEPOWER) ||
#ifndef RENEWAL
...
void status_calc_state_sub(
	block_list &bl,
	status_change &sc,
	bool start,
	std::shared_ptr<s_status_change_db> scdb_main,
	bool &restriction,
	e_scs_flag flag,
	e_scs_flag flag_conditional,
	std::function<bool(block_list &, status_change &, bool &, const sc_type, const status_change_entry &)>
		func_switch) {
	...
	for (const auto &it : sc) {
		...
		if (!func_switch(bl, sc, restriction, it.first, it.second)) {
			...
			ShowError(
				"status_calc_state_sub: status \"%s\" is defined with \"%s\", but the condition is not implemented.\n",
				constant_sc,
				constant_scs);
			continue;
		}
...
void status_calc_state(block_list &bl, status_change &sc, std::shared_ptr<s_status_change_db> scdb, bool start) {
	...
	static std::function<bool(block_list &, status_change &, bool &, const sc_type, const status_change_entry &)>
		func_not_impl = [](block_list &bl,
						   status_change &sc,
						   bool &restriction,
						   const sc_type type,
						   const status_change_entry &sce) -> bool {
	...
	if (scdb->state[SCS_NOMOVE] || scdb->state[SCS_NOMOVECOND]) {
		status_calc_state_sub(
			...
			SCS_NOMOVE,
			SCS_NOMOVECOND,
			[](block_list &bl, status_change &sc, bool &restriction, const sc_type type, const status_change_entry &sce)
				-> bool {
				...
				switch (type) {
					...
					case SC_DANCING:
						if (sce.val4 != 0) {
							...
							// Unless you have SC_LONGING, then the restiction only applies for CG_MOONLIT and
							// CG_HERMODE
							if (sc.getSCE(SC_LONGING) != nullptr && (sce.val1 & 0xFFFF) != CG_MOONLIT &&
								(sce.val1 & 0xFFFF) != CG_HERMODE) {
								break;
							}
#endif
							restriction = true;
					...
					case SC_LONGING:
						// Does not do anything. SC_LONGING is just defined with SCS_NOMOVECOND to trigger a
						// recalculation with the conditions under SC_DANCING above
						break;
		...
		status_calc_state_sub(
			...
			[](block_list &bl, status_change &sc, bool &restriction, const sc_type type, const status_change_entry &sce)
				...
				switch (type) {
					...
					case SC_LONGING:
						// Does not do anything. SC_LONGING is just defined with SCS_NOATTACKCOND to trigger a
						// recalculation with the conditions under SC_DANCING above
						break;
	...
	if (bl.type == BL_PC) {
		...
		if (scdb->state[SCS_NOEQUIPITEM] || scdb->state[SCS_NOEQUIPITEMCOND]) {
			status_calc_state_sub(
				bl, sc, start, scdb, sc.cant.equip, SCS_NOEQUIPITEM, SCS_NOEQUIPITEMCOND, func_not_impl);
		}

		// Can't unequip item
		if (scdb->state[SCS_NOUNEQUIPITEM] || scdb->state[SCS_NOUNEQUIPITEMCOND]) {
			status_calc_state_sub(
				bl, sc, start, scdb, sc.cant.unequip, SCS_NOUNEQUIPITEM, SCS_NOUNEQUIPITEMCOND, func_not_impl);
		}
		...
		if (scdb->state[SCS_NODEATHPENALTY] || scdb->state[SCS_NODEATHPENALTYCOND]) {
			status_calc_state_sub(
				bl, sc, start, scdb, sc.cant.deathpenalty, SCS_NODEATHPENALTY, SCS_NODEATHPENALTYCOND, func_not_impl);
		}

		// Can't sit/stand/talk to NPC
		if (scdb->state[SCS_NOINTERACT] || scdb->state[SCS_NOINTERACTCOND]) {
			status_calc_state_sub(
				bl, sc, start, scdb, sc.cant.interact, SCS_NOINTERACT, SCS_NOINTERACTCOND, func_not_impl);
		}
	}
}
...
int32 status_get_lv(const block_list *bl) {
	...
	switch (bl->type) {
		case BL_PC:
			return static_cast<const TBL_PC *>(bl)->status.base_level;
		case BL_MOB:
			return static_cast<const TBL_MOB *>(bl)->level;
		case BL_PET:
			return static_cast<const TBL_PET *>(bl)->pet.level;
		case BL_HOM:
			return static_cast<const TBL_HOM *>(bl)->homunculus.level;
		case BL_MER:
			return static_cast<const TBL_MER *>(bl)->db->lv;
		case BL_ELEM:
			return static_cast<const TBL_ELEM *>(bl)->db->lv;
		case BL_NPC:
			return static_cast<const TBL_NPC *>(bl)->level;
	}
	return 1;
}
	...
	switch (bl->type) {
		...
		case BL_MOB: {
			...
			if (vd) {
				...
				md->vd->look[LOOK_HAIR_COLOR] =
					cap_value(md->vd->look[LOOK_HAIR_COLOR], MIN_HAIR_COLOR, MAX_HAIR_COLOR);
			}
			else {
...
const status_change *status_get_sc(const block_list *bl) {
	if (bl) {
		switch (bl->type) {
			case BL_PC:
				return &static_cast<const TBL_PC *>(bl)->sc;
			case BL_MOB:
				return &static_cast<const TBL_MOB *>(bl)->sc;
			case BL_NPC:
				return &static_cast<const TBL_NPC *>(bl)->sc;
			case BL_HOM:
				return &static_cast<const TBL_HOM *>(bl)->sc;
			case BL_MER:
				return &static_cast<const TBL_MER *>(bl)->sc;
			case BL_ELEM:
				return &static_cast<const TBL_ELEM *>(bl)->sc;
		}
	}
	return nullptr;
}
...
status_change *status_get_sc(block_list *bl) {
	if (bl) {
		switch (bl->type) {
			case BL_PC:
				return &static_cast<TBL_PC *>(bl)->sc;
			case BL_MOB:
				return &static_cast<TBL_MOB *>(bl)->sc;
			case BL_NPC:
				return &static_cast<TBL_NPC *>(bl)->sc;
			case BL_HOM:
				return &static_cast<TBL_HOM *>(bl)->sc;
			case BL_MER:
				return &static_cast<TBL_MER *>(bl)->sc;
			case BL_ELEM:
				return &static_cast<TBL_ELEM *>(bl)->sc;
		}
	}
	return nullptr;
}
...
TIMER_FUNC(status_change_start_timer) {
	...
	if (bl != nullptr && !status_isdead(*bl)) {
		status_change_start_post_delay(
			src, bl, entry->type, entry->val1, entry->val2, entry->val3, entry->val4, entry->tick, entry->flag);
	}
 ...
 */
bool status_change_start(block_list *src,
						 block_list *bl,
						 sc_type type,
						 int32 rate,
						 int32 val1,
			...
			case SC_ENDURE:
				...
				if (!(flag & SCSTART_NOAVOID) && (bl->type & (BL_PC | BL_MER)) && !map_flag_gvg2(bl->m) &&
					...
					if (sd) {
						...
						status_change_start(
							src, tsd, type, 10000, val1, val2, val3, val4, tick, SCSTART_NOAVOID | SCSTART_NOICON);
					}
				}
				if (val4) {
					tick = INFINITE_TICK;
				...
				if (!(flag & SCSTART_NOAVOID) && (bl->type & (BL_PC | BL_MER))) {
					...
					if (sd) {
						...
						for (i = 0; i < MAX_DEVOTION; i++) {
							if (sd->devotion[i] && (tsd = map_id2sd(sd->devotion[i]))) {
								status_change_start(
									src, tsd, type, 10000, val1, val2, 0, 1, tick, SCSTART_NOAVOID | SCSTART_NOICON);
							}
						}
					}
					else if (bl->type == BL_MER && ((TBL_MER *)bl)->devotion_flag && (tsd = ((TBL_MER *)bl)->master)) {
				...
				if (!(flag & SCSTART_NOAVOID)) {
					...
					if (bl->type & (BL_PC | BL_MER)) {
						if (sd) {
							...
							status_change_start(
								src, tsd, type, 10000, val1, val2, 0, 0, tick, SCSTART_NOAVOID | SCSTART_NOICON);
						}
					}
				}
				break;
				...
				if (!(flag & SCSTART_NOAVOID)) {
					...
					if (sd) {
						...
						for (i = 0; i < MAX_DEVOTION;
							...
							if (sd->devotion[i] && (tsd = map_id2sd(sd->devotion[i]))) {
								status_change_start(
									src, tsd, type, 10000, val1, val2, val3, val4, tick, SCSTART_NOAVOID);
							}
						}
					}
				}
...
int32 status_change_end(struct block_list *bl, enum sc_type type, int32 tid) {
	...
	switch (type) {
		...
		case SC_RUN: {
			...
			if (begin_spurt && val1 >= 7 && DIFF_TICK(gettick(), starttick) <= 1000 &&
		...
		case SC_CLOSECONFINE:
			if (val2 > 0) {
				...
				int32 range =
					1 + skill_get_range2(bl, scdb->skill_id, val1, true) +
		...
		case SC_CURSEDCIRCLE_ATKER:
			if (val2) { // Used the default area size cause there is a chance the caster could knock back and can't
						// clear the target.
				map_foreachinallrange(status_change_timer_sub,
									  bl,
									  AREA_SIZE + 3,
									  BL_CHAR,
									  bl,
	...
	switch (type) {
		...
		case SC_SERVICE4U:
			if (battle_config.refresh_song == 0 && !status_isdead(*bl)) {
				// Most songs have no interval, but they can expire while the character is still standing in the area of
				// effect We need to make sure to restore the duration here if that's the case
				std::shared_ptr<s_status_change_db> scdb = status_db.find(type);
				if (scdb == nullptr) {
					break;
				}
				...
				if (unit->group->src_id == bl->id &&
					!(sc != nullptr && sc->getSCE(SC_SPIRIT) && sc->getSCE(SC_SPIRIT)->val2 == SL_BARDDANCER)) {
					break;
				}
		...
		case SC_TALISMAN_OF_PROTECTION:
			if (--(sce->val4) >= 0) {
				...
				if (ssd == nullptr || status_isdead(*ssd) || ssd->m != bl->m || sd == nullptr ||
	...
	for (auto &entry : *this) {
		...
		for (const auto &scs_check : scs_checks) {
			if (status->state[scs_check.conditional] && status->state[scs_check.unconditional]) {
				...
				if (constant != nullptr) {
					ShowWarning(
						"StatusDatabase::loadingFinished: %s and %s defined for status change \"%s\". Removing "
						"unconditional...\n",
						scs_check.conditional_str,
						scs_check.unconditional_str,
						constant);
				}
				else {
					ShowWarning(
						"StatusDatabase::loadingFinished: %s and %s defined for status change \"%d\". Removing "
						"unconditional...\n",
						scs_check.conditional_str,
						scs_check.unconditional_str,
						entry.first);
