		...
		for (const ryml::NodeRef& itemNode : node["Items"]) {
			...
			if (this->nodeExists(itemNode, "Refine")) {
				...
				if (data->flag.no_refine) {
					this->invalidWarning(
						itemNode["Refine"], "barter_parseBodyNode: Item %s is not refineable.\n", data->name.c_str());
					return 0;
				}
...
bool npc_enable_target(npc_data& nd, uint32 char_id, e_npcv_status flag) {
	if (char_id > 0 && (flag & NPCVIEW_CLOAK)) {
		...
		map_foreachinmap(npc_cloaked_sub,
						 nd.m,
						 BL_PC,
						 nd.id); // Because npc option has been updated we remove the npc id from sd->cloaked_npc
	}
...
TIMER_FUNC(npc_secure_timeout_timer) {
	...
	if (DIFF_TICK(cur_tick, sd->npc_idle_tick) > (timeout * 1000)) {
		...
		sd->npc_idle_timer =
			add_timer(cur_tick + (SECURE_NPCTIMEOUT_INTERVAL * 1000), npc_secure_timeout_timer, sd->id, 0);
	}
	return 0;
}
#endif
...
void npc_timerevent_quit(map_session_data* sd) {
	...
	if (nd && nd->type == BL_NPC) {
		...
		if (ev) {
			...
			old_rid =
				(nd->u.scr.rid == sd->id ? 0 : nd->u.scr.rid); // Detach rid if the last attached player logged off.
			old_tick = nd->u.scr.timertick;
			old_timer = nd->u.scr.timer;
...
int32 npc_event_sub(map_session_data* sd, struct event_data* ev, const char* eventname) {
	...
	if (strcmp(eventname, ontouch_event_name) == 0 || strcmp(eventname, ontouch2_event_name) == 0) {
		...
		if (x > 0 && y > 0 && (xs > -1 && ys > -1) &&
				((sd->x < x - xs) || (sd->x > x + xs) || (sd->y < y - ys) || (sd->y > y + ys)) ||
		...
		std::remove_if(
			...
			[&](const int32& current_npc_id) {
				...
				if (!nd) {
					...
					if (sd->m != nd->m || sd->x < nd->x - xs || sd->x > nd->x + xs || sd->y < nd->y - ys ||
						sd->y > nd->y + ys || leavemap) {
						char name[EVENT_NAME_LENGTH];
...
int32 npc_touch_area_allnpc(map_session_data* sd, int16 m, int16 x, int16 y) {
	...
	for (size_t i = 0; i < sd->areanpc.size(); i++) {
		...
		if (!nd || nd->subtype != NPCTYPE_SCRIPT ||
			!(nd->m == m && x >= nd->x - nd->u.scr.xs && x <= nd->x + nd->u.scr.xs && y >= nd->y - nd->u.scr.ys &&
			  y <= nd->y + nd->u.scr.ys)) {
...
int32 npc_touch_areanpc2(mob_data* md) {
	...
	for (i = 0; i < mapdata->npc_num_area; i++) {
		...
		if (x >= mapdata->npc[i]->x - xs && x <= mapdata->npc[i]->x + xs && y >= mapdata->npc[i]->y - ys &&
			y <= mapdata->npc[i]->y + ys) { // In the npc touch area
			switch (mapdata->npc[i]->subtype) {
				case NPCTYPE_WARP: {
					...
					if (unit_warp(md, warp_m, mapdata->npc[i]->u.warp.x, mapdata->npc[i]->u.warp.y, CLR_OUTSIGHT) ==
						0) {
						return 1; // Warped
					}
				} break;
...
int32 npc_check_areanpc(int32 flag, int16 m, int16 x, int16 y, int16 range) {
	...
	for (i = 0; i < mapdata->npc_num_area; i++) {
		...
		if (x1 >= mapdata->npc[i]->x - xs && x0 <= mapdata->npc[i]->x + xs && y1 >= mapdata->npc[i]->y - ys &&
			y0 <= mapdata->npc[i]->y + ys) {
			break; // found a npc
		}
	}
...
struct npc_data* npc_checknear(map_session_data* sd, struct block_list* bl) {
	...
	if (bl->m != sd->m || bl->x < sd->x - AREA_SIZE - 1 || bl->x > sd->x + AREA_SIZE + 1 ||
	...
	switch (mapflag) {
		...
		case MF_INVINCIBLE_TIME: {
			...
			if (state && sscanf(w4, "%11d", &args.flag_val) != 1) {
				ShowError(
					"npc_parse_mapflag: invincible_time: Invalid time '%s' for Invincible Time mapflag. Skipping (file "
					"'%s', line '%d')\n",
					w4,
					filepath,
					strline(buffer, start - buffer));
...
struct npc_data* npc_duplicate_npc_for_player(struct npc_data& nd, map_session_data& sd) {
	...
	struct npc_data* dnd =
		npc_duplicate_npc(nd, nd.name, sd.m, new_x, new_y, nd.class_, dir, nd.u.scr.xs, nd.u.scr.ys, &sd);
