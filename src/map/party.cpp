...
void party_send_movemap(map_session_data* sd) {
	...
	if (sd->fd) { // synchronize minimap positions with the rest of the party
		int32 i;
		for (i = 0; i < MAX_PARTY; i++) {
			if (p->data[i].sd && p->data[i].sd != sd && p->data[i].sd->m == sd->m) {
...
void party_exp_share(struct party_data* p, struct block_list* src, t_exp base_exp, t_exp job_exp, int32 zeny) {
	...
	for (i = c = 0; i < MAX_PARTY; i++) {
		if ((sd[c] = p->data[i].sd) == nullptr || sd[c]->m != src->m || pc_isdead(sd[c]) ||
	...
	if (p && p->party.item & 2 && (first_charid || !(battle_config.party_share_type & 1))) {
		...
		if (battle_config.party_share_type & 2) { // Round Robin
			...
			do {
				...
				if ((psd = p->data[i].sd) == nullptr || sd->m != psd->m || pc_isdead(psd) ||
			...
			for (i = 0; i < MAX_PARTY; i++) {
				if ((psd[count] = p->data[i].sd) == nullptr || psd[count]->m != sd->m || pc_isdead(psd[count]) ||
...
int32 party_foreachsamemap(int32 (*func)(struct block_list*, va_list), map_session_data* sd, int32 range, ...) {
	...
	for (i = 0; i < MAX_PARTY; i++) {
		...
		if (range && (psd->x < x0 || psd->y < y0 || psd->x > x1 || psd->y > y1)) {
			continue;
		}
...
void party_booking_search(
	map_session_data* sd, int16 level, int16 mapid, int16 job, unsigned long lastindex, int16 resultcount) {
	struct party_booking_ad_info* pb_ad;
	int32 i, count = 0;
