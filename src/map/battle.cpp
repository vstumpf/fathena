...
int32 battle_damage(struct block_list *src,
	...
	// must be placed here These events trigger when the attack connects
...
static int32 battle_calc_sizefix(
	int64 damage, map_session_data *sd, unsigned char t_size, unsigned char weapon_type, int16 flag) {
		...
		if (!skill_id || skill_id == KN_AUTOCOUNTER) {
			if (status_change_entry *sce = sc->getSCE(SC_POISONREACT); sce != nullptr && sce->val4 == 1) {
				...
				sc_start2(src,
						  ...
						  sce->val1,
						  src->id,
						  skill_get_time2(AS_POISONREACT, sce->val1),
						  sstatus->amotion);
...
static void battle_calc_weapon_final_atk_modifiers(
	...
	if (tsc != nullptr && wd->damage > 0) {
		if (status_change_entry *sce = tsc->getSCE(SC_POISONREACT);
			sce != nullptr && rnd_chance_official(sce->val3, 100)) {
...
static struct Damage battle_calc_weapon_attack(
	...
	if (!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true)) {
		...
		if ((wd.damage + wd.damage2) && tstatus->res > 0 && !nk[NK_SIMPLEDEFENSE]) {
			...
			wd.damage -= static_cast<decltype(wd.damage)>(static_cast<float>(res / (res + 400.0)) * 80.0 / 100.0 *
														  static_cast<double>(wd.damage));
			wd.damage2 -= static_cast<decltype(wd.damage2)>(static_cast<float>(res / (res + 400.0)) * 80.0 / 100.0 *
															static_cast<double>(wd.damage2));
		}
...
#define MATK_ADD(a) \
	{ \
		...
		if (ad.damage != 0 && tstatus->mres > 0) {
			...
			ad.damage -= static_cast<decltype(ad.damage)>(static_cast<float>(mres / (mres + 400.0)) * 80.0 / 100.0 *
														  static_cast<double>(ad.damage));
		}
#endif
...
bool battle_check_coma(map_session_data &sd, struct block_list &target, e_battle_flag attack_type) {
	...
	// Coma
	if (sd.special_state.bonus_coma && (!dstmd || (!util::vector_exists(status_get_race2(dstmd), RC2_GVG) &&
												   status_get_class(dstmd) != CLASS_BATTLEFIELD))) {
		int32 rate = 0;
...
enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, t_tick tick, int32 flag) {
	...
	if (target->type == BL_SKILL && damage > 0) {
		...
		// Monsters trigger a special event when they are hit in Berserk state by a normal attack within [attack
		// range+1] Certain AI types only switch target on this trigger This event only triggers if the target is not
		// already the current attacker We need to do this here because it needs to be calculated before waiting for
		// attack motion
		mob_data &md = *reinterpret_cast<mob_data *>(target);
		if (md.state.skillstate == MSS_BERSERK && md.target_id != src->id) {
			is_norm_attacked =
				((battle_config.mob_ai & 0x4) || check_distance_bl(src, target, md.status.rhw.range + 1));
		}
	}
...
void battle_adjust_conf() {
	...
	battle_config.max_third_aspd =
		(AMOTION_ZERO_ASPD - battle_config.max_third_aspd * AMOTION_INTERVAL) * AMOTION_DIVIDER_PC;
	battle_config.max_summoner_aspd =
		(AMOTION_ZERO_ASPD - battle_config.max_summoner_aspd * AMOTION_INTERVAL) * AMOTION_DIVIDER_PC;
	battle_config.max_extended_aspd =
		(AMOTION_ZERO_ASPD - battle_config.max_extended_aspd * AMOTION_INTERVAL) * AMOTION_DIVIDER_PC;
	...
	#if !defined(BUILDBOT)
		ShowWarning(
			"conf/battle/feature.conf:mesitemicon is enabled but it requires PACKETVER 2023-03-02 or newer, "
			"disabling...\n");
	#endif
