...
static inline void RBUFPOS2(
	const uint8* p, uint16 pos, int16* x0, int16* y0, int16* x1, int16* y1, unsigned char* sx0, unsigned char* sy0) {
	p += pos;
...
static inline void RFIFOPOS2(
	int32 fd, uint16 pos, int16* x0, int16* y0, int16* x1, int16* y1, unsigned char* sx0, unsigned char* sy0) {
			...
			if (p) {
				for (i = 0; i < MAX_PARTY; i++) {
					...
					if ((type == PARTY_AREA || type == PARTY_AREA_WOS) &&
						(sd->x < x0 || sd->y < y0 || sd->x > x1 || sd->y > y1)) {
						continue;
					}
...
void clif_scriptmenu(map_session_data& sd, uint32 npcid, const char* mes) {
	...
	if (!sd.state.using_fake_npc &&
		(npcid == fake_nd->id ||
		 ((bl = map_id2bl(npcid)) && (bl->m != sd.m || bl->x < sd.x - AREA_SIZE - 1 || bl->x > sd.x + AREA_SIZE + 1 ||
...
void clif_parse_guild_castle_teleport_request(int32 fd, map_session_data* sd) {
	...
	if (map_getmapflag(sd->m, MF_GVG_CASTLE) || map_getmapflag(sd->m, MF_NOTELEPORT) ||
...
static int32 clif_hpmeter(map_session_data* sd) {
	...
	map_foreachinallarea(
		clif_hpmeter_sub, sd->m, sd->x - AREA_SIZE, sd->y - AREA_SIZE, sd->x + AREA_SIZE, sd->y + AREA_SIZE, BL_PC, sd);
	return 0;
}
...
static enum e_damage_type clif_calc_delay(
	block_list& bl, e_damage_type type, int32 div, int64 damage, int32 delay, t_tick tick) {
	if (damage < 1) {
		return type;
	}
...
void clif_damage(block_list& src,
				 block_list& dst,
				 t_tick tick,
				 int32 sdelay,
				 int32 ddelay,
...
void clif_addskill(map_session_data& sd, uint16 skill_id) {
	...
	p.skill.range2 =
		static_cast<decltype(p.skill.range2)>(skill_get_range2(&sd, skill_id, sd.status.skill[idx].lv, false));
...
/// 0b1a <src id>.L <dst id>.L <x>.W <y>.W <skill id>.W <property>.L <delaytime>.L <is disposable>.B <attackMT>.L
/// (ZC_USESKILL_ACK3) property:
///     0 = Yellow cast aura
...
///     1 = no text
void clif_skillcasting(block_list& src,
					   block_list* dst,
					   uint16 dst_x,
					   uint16 dst_y,
					   uint16 skill_id,
...
void clif_skill_damage(block_list& src,
					   block_list& dst,
					   t_tick tick,
					   int32 sdelay,
					   int32 ddelay,
	...
	auto damage =
		std::min(static_cast<decltype(packet.damage)>(sdamage), std::numeric_limits<decltype(packet.damage)>::max());
...
void clif_skill_estimation(map_session_data& sd, mob_data& md) {
	...
	packet.def = static_cast<decltype(packet.def)>(((battle_config.estimation_type & 1) ? md.status.def : 0) +
												   ((battle_config.estimation_type & 2) ? md.status.def2 : 0));
	packet.race = md.status.race;
	packet.mdef = static_cast<decltype(packet.mdef)>(((battle_config.estimation_type & 1) ? md.status.mdef : 0) +
													 ((battle_config.estimation_type & 2) ? md.status.mdef2 : 0));
	packet.element = md.status.def_ele;
	// The following caps negative attributes to 0 since the client displays them as 255-fix. [Skotlex]
	packet.water = static_cast<decltype(packet.water)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_WATER, md.status.def_ele), (int16)0));
	packet.earth = static_cast<decltype(packet.earth)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_EARTH, md.status.def_ele), (int16)0));
	packet.fire = static_cast<decltype(packet.fire)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_FIRE, md.status.def_ele), (int16)0));
	packet.wind = static_cast<decltype(packet.wind)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_WIND, md.status.def_ele), (int16)0));
	packet.poison = static_cast<decltype(packet.poison)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_POISON, md.status.def_ele), (int16)0));
	packet.holy = static_cast<decltype(packet.holy)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_HOLY, md.status.def_ele), (int16)0));
	packet.shadow = static_cast<decltype(packet.shadow)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_DARK, md.status.def_ele), (int16)0));
	packet.ghost = static_cast<decltype(packet.ghost)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_GHOST, md.status.def_ele), (int16)0));
	packet.undead = static_cast<decltype(packet.undead)>(
		std::max(elemental_attribute_db.getAttribute(md.status.ele_lv, ELE_UNDEAD, md.status.def_ele), (int16)0));
...
void clif_efst_status_change(block_list& bl,
							 block_list& tbl,
							 enum send_target target,
							 efst_type type,
							 t_tick tick,
...
void clif_sendegg(map_session_data* sd) {
	...
	if (battle_config.pet_no_gvg &&
		map_flag_gvg2(sd->m)) { // Disable pet hatching in GvG grounds during Guild Wars [Skotlex]
...
void clif_guild_positionchanged(const struct mmo_guild& g, int32 idx) {
	...
	PACKET_ZC_ACK_CHANGE_GUILD_POSITIONINFO* p =
		reinterpret_cast<PACKET_ZC_ACK_CHANGE_GUILD_POSITIONINFO*>(packet_buffer);
...
inline void clif_pk_mode_message(map_session_data* sd) {
	if (battle_config.pk_mode && battle_config.pk_mode_mes && sd && map_getmapflag(sd->m, MF_PVP)) {
...
void clif_parse_LoadEndAck(int32 fd, map_session_data* sd) {
	...
	if (sd->state.connect_new) {
		...
		if (battle_config.friend_auto_add) {
			...
			map_foreachpc(
				clif_friendslist_toggle_sub, sd->status.account_id, sd->status.char_id, static_cast<int32>(true));
		}
...
#ifdef VIP_ENABLE
		if (!sd->state.connect_new && !sd->vip.disableshowrate && sd->state.pmap != sd->m &&
...
void clif_parse_WalkToXY(int32 fd, map_session_data* sd) {
	...
	if (battle_config.official_cell_stack_limit > 0 && sd->x == x + 1 && sd->y == y &&
		map_count_oncell(sd->m, x, y, BL_CHAR | BL_NPC, 1)) {
		return;
	}
...
void clif_parse_GetItemFromCart(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_MOVE_ITEM_FROM_CART_TO_BODY* p =
		reinterpret_cast<PACKET_CZ_MOVE_ITEM_FROM_CART_TO_BODY*>(RFIFOP(fd, 0));
...
static void clif_parse_UseSkillToPosSub(
	int32 fd, map_session_data& sd, uint16 skill_lv, uint16 skill_id, int16 x, int16 y, int32 skillmoreinfo) {
	t_tick tick = gettick();
...
void clif_parse_ItemIdentify(int32 fd, map_session_data* sd) {
	...
	if (idx >= MAX_INVENTORY || sd->inventory.u.items_inventory[idx].nameid == 0 ||
...
void clif_parse_MoveToKafraFromCart(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_MOVE_ITEM_FROM_CART_TO_STORE* p =
		reinterpret_cast<PACKET_CZ_MOVE_ITEM_FROM_CART_TO_STORE*>(RFIFOP(fd, 0));
	uint16 idx = server_index(p->index);
...
void clif_parse_MoveFromKafraToCart(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_MOVE_ITEM_FROM_STORE_TO_CART* p =
		reinterpret_cast<PACKET_CZ_MOVE_ITEM_FROM_STORE_TO_CART*>(RFIFOP(fd, 0));
...
void clif_parse_GuildChangeEmblem(int32 fd, map_session_data* sd) {
	...
	if (!(inter_config.emblem_woe_change) && is_agit_start()) {
		clif_messagecolor(
			sd, color_table[COLOR_RED], msg_txt(sd, 385), false, SELF); //"You not allowed to change emblem during woe"
		return;
	}
	...
	if (emb_val == EMBVALIDATE_ERR_RAW_FILEFORMAT) {
		ShowWarning(
			...
			sd->status.guild_id);
		clif_messagecolor(
			sd, color_table[COLOR_RED], msg_txt(sd, 386), false, SELF); //"The chosen emblem was detected invalid\n"
		return;
	}
...
void clif_roulette_generate(
	map_session_data* sd, unsigned char result, int16 stage, int16 prizeIdx, t_itemid bonusItemID) {
	nullpo_retv(sd);
...
void clif_parse_private_airship_request(int32 fd, map_session_data* sd) {
	...
	if (!battle_config.feature_privateairship) {
		clif_messagecolor(
			sd, color_table[COLOR_RED], msg_txt(sd, 792), false, SELF); // The private airship system is disabled.
		return;
	}
