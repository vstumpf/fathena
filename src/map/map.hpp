...
skill_unit *map_find_skill_unit_oncell(
	struct block_list *, int16 x, int16 y, uint16 skill_id, skill_unit *, int32 flag);
// search and creation
...
#define BL_CAST(type_, bl) \
	(((bl) == nullptr || (bl)->type != (type_)) \
		 ? nullptr \
		 : static_cast<typename std::conditional<std::is_const<std::remove_pointer_t<decltype(bl)>>::value, \
												 const T##type_ *, \
												 T##type_ *>::type>(bl))
