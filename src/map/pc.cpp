...
void pc_reg_received(map_session_data *sd) {
	...
	if (sd->state.connect_new == 0 && sd->fd) { // Character already loaded map! Gotta trigger LoadEndAck manually.
		...
		if (pc_isinvisible(sd)) {
			...
			if (map_getmapflag(sd->m, MF_PVP) && !map_getmapflag(sd->m, MF_PVP_NOCALCRANK) &&
				sd->pvp_timer != INVALID_TIMER) {
...
void pc_updateweightstatus(map_session_data &sd) {
	...
	uint8 new_overweight = (overweight_percent >= battle_config.major_overweight_rate)      ? 2
						   : (overweight_percent >= battle_config.natural_heal_weight_rate) ? 1
																							: 0;
...
static void pc_bonus_autospell_onskill(
	std::vector<s_autospell> &spell, uint16 src_skill, uint16 id, uint16 lv, int16 rate, t_itemid card_id, uint8 flag) {
	if (spell.size() == MAX_PC_BONUS) {
...
static void pc_bonus_item_drop(
	std::vector<s_add_drop> &drop, t_itemid nameid, uint16 group, int32 class_, int16 race, int32 rate) {
	if (!nameid && !group) {
 ...
 *   7 = stack limitation
 */
enum e_additem_result pc_additem(
	map_session_data *sd, struct item *item, int32 amount, e_log_pick_type log_type, bool favorite) {
	struct item_data *id;
	int16 i;
	uint32 w;
	...
	if (itemdb_group.item_exists(IG_GIANT_FLY_WING, nameid)) {
		...
		if (pd) {
			...
			ARR_FIND(
				...
				MAX_PARTY,
				i,
				pd->data[i].sd && pd->data[i].sd != sd && pd->data[i].sd->m == sd->m && !pc_isdead(pd->data[i].sd));
...
int32 pc_useitem(map_session_data *sd, int32 n) {
	...
	if (id->flag.delay_consume > 0 &&
		(sd->ud.skilltimer != INVALID_TIMER /*|| !status_check_skilluse(sd, sd, ALL_RESURRECTION, 0)*/)) {
		return 0;
	}
...
bool pc_steal_item(map_session_data *sd, struct block_list *bl, uint16 skill_lv) {
	...
	if (md->master_id || status_has_mode(md_status, MD_STATUSIMMUNE) ||
		util::vector_exists(status_get_race2(md), RC2_TREASURE) ||
...
enum e_setpos pc_setpos(map_session_data *sd, uint16 mapindex, int32 x, int32 y, clr_type clrtype) {
	...
	if (sd->state.changemap) { // Misc map-changing settings
		int32 curr_map_instance_id = map_getmapdata(sd->m)->instance_id,
...
char pc_randomwarp(map_session_data *sd, clr_type type, bool ignore_mapflag) {
	...
	} while ((map_getcell(sd->m, x, y, CELL_CHKNOPASS) ||
			  (!battle_config.teleport_on_portal && npc_check_areanpc(1, sd->m, x, y, 1))) &&
			 (i++) < 1000);
...
bool pc_memo(map_session_data *sd, int32 pos) {
	...
	// check mapflags
	if (sd->m >= 0 && (map_getmapflag(sd->m, MF_NOMEMO) || map_getmapflag(sd->m, MF_NOWARPTO)) &&
...
TIMER_FUNC(pc_follow_timer) {
	...
	if (sd->prev != nullptr && tbl->prev != nullptr && sd->ud.skilltimer == INVALID_TIMER &&
...
int64 pc_readparam(map_session_data *sd, int64 type) {
	...
	switch (type) {
		...
		case SP_CRITICAL:
			...
			break;
		case SP_ASPD:
			val = (AMOTION_ZERO_ASPD - sd->battle_status.amotion) / AMOTION_INTERVAL;
			break;
		case SP_BASE_ATK:
#ifdef RENEWAL
			val = sd->bonus.eatk;
...
bool pc_jobchange(map_session_data *sd, int32 job, char upper) {
	...
	clif_changelook(
		sd,
		LOOK_BASE,
		sd->vd.look[LOOK_BASE]); // move sprite update to prevent client crashes with incompatible equipment [Valaris]
#if PACKETVER >= 20151001
	clif_changelook(
		sd, LOOK_HAIR, sd->vd.look[LOOK_HAIR]); // Update player's head (only matters when switching to or from Doram)
#endif
			...
			for (i = 0; i < MAX_SLOTS; i++) {
				...
				if (data != nullptr) {
					if (data->equip_script &&
						(pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(data.get(), sd->m))) {
...
uint16 pc_level_penalty_mod(map_session_data *sd, e_penalty_type type, std::shared_ptr<s_mob_db> mob, mob_data *md) {
	...
	if ((type == PENALTY_DROP && map_getmapflag(sd->m, MF_NORENEWALDROPPENALTY)) ||
		(type == PENALTY_EXP && map_getmapflag(sd->m, MF_NORENEWALEXPPENALTY))) {
		return 100;
	}
...
void pc_expire_check(map_session_data *sd) {
	...
	sd->expiration_tid =
		add_timer(gettick() + (uint32)(sd->expiration_time - time(nullptr)) * 1000, pc_expiration_timer, sd->id, 0);
}
...
enum e_BANKING_WITHDRAW_ACK pc_bank_withdraw(map_session_data *sd, int32 money) {
	...
	if (money <= 0) {
		...
		clif_messagecolor(
			sd, color_table[COLOR_RED], msg_txt(sd, 1495), false, SELF); // You can't withdraw that much money
		return BWA_UNKNOWN_ERROR;
	}
...
static void pc_macro_punishment(map_session_data &sd, e_macro_detect_status stype) {
	...
	if (battle_config.macro_detection_punishment == 0) { // Ban
		...
		chrif_req_login_operation(
			reporter_aid, sd.status.name, (duration == 0 ? CHRIF_OP_LOGIN_BLOCK : CHRIF_OP_LOGIN_BAN), duration, 0, 0);
	}
	else { // Jail
...
void pc_macro_captcha_register(map_session_data &sd,
							   uint16 image_size,
							   const char captcha_answer[CAPTCHA_ANSWER_SIZE_MAX]) {
...
TIMER_FUNC(pc_macro_detector_timeout) {
	...
	if (sd->macro_detect.retry == 0) {
		...
		sd->macro_detect.timer =
			add_timer(gettick() + battle_config.macro_detection_timeout, pc_macro_detector_timeout, sd->id, 0);
	}
	return 0;
}
...
void pc_macro_reporter_area_select(map_session_data &sd, const int16 x, const int16 y, const int8 radius) {
	...
	map_foreachinarea(
		pc_macro_reporter_area_select_sub, sd.m, x - radius, y - radius, x + radius, y + radius, BL_PC, &aid_list);
