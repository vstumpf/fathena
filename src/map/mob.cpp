...
bool mob_ksprotected(struct block_list *src, struct block_list *target) {
	...
	do {
		...
		if (t_sd && ((sce->val2 == 1 && sce->val1 != t_sd->id) ||
...
mob_data *mob_once_spawn_sub(struct block_list *bl,
							 int16 m,
							 int16 x,
							 int16 y,
							 const char *mobname,
...
int32 mob_once_spawn(map_session_data *sd,
	...
	for (count = 0; count < amount; count++) {
		...
		if (mob_id < 0 && battle_config.dead_branch_active) {
			...
			sc_start4(
				nullptr, md, SC_MODECHANGE, 100, 1, 0, MD_AGGRESSIVE | MD_CANATTACK | MD_CANMOVE | MD_ANGRY, 0, 60000);
		}
	}
...
int32 mob_spawn_guardian(const char *mapname,
	...
	if (has_index && gc->guardian[guardian].id) { // Check if guardian already exists, refuse to spawn if so.
		...
		if (md2 && md2->type == BL_MOB && md2->guardian_data && md2->guardian_data->number == guardian) {
			ShowError(
	...
	if (g) {
		...
		add_timer(
			gettick() + battle_config.mob_respawn_time, mob_spawn_guardian_sub, md->id, md->guardian_data->guild_id);
	}
	mob_spawn(md);
...
int32 mob_linksearch(struct block_list *bl, va_list ap) {
	...
	if (md->mob_id == mob_id && status_has_mode(&md->status, MD_ASSIST) &&
		DIFF_TICK(tick, md->last_linktime) >= MIN_MOBLINKTIME && md->target_id == 0) {
	...
	if (md->spawn) { // Respawn data
		...
		if ((!md->spawn->state.boss || (md->x == 0 && md->y == 0) || md->spawn->xs != 1 || md->spawn->ys != 1) &&
			(md->spawn->xs + md->spawn->ys < 1 || !rnd_chance(1, md->spawn->xs * md->spawn->ys) ||
			 !map_getcell(md->m, md->x, md->y, CELL_CHKREACH))) {
			...
			if (!map_search_freecell(md,
				...
				// tries)
				if (!map_getcell(md->m, md->x, md->y, CELL_CHKREACH) &&
					!map_search_freecell(md, -1, &md->x, &md->y, -1, -1, battle_config.no_spawn_on_player ? 4 : 0)) {
					// Retry again later
					if (md->spawn_timer != INVALID_TIMER) {
						delete_timer(md->spawn_timer, mob_delayspawn);
					}
		...
		if (battle_config.no_spawn_on_player > 99 &&
			map_foreachinallrange(mob_count_sub, md, AREA_SIZE, BL_PC)) { // retry again later (players on sight)
			if (md->spawn_timer != INVALID_TIMER) {
				delete_timer(md->spawn_timer, mob_delayspawn);
			}
			md->spawn_timer = add_timer(tick + battle_config.mob_respawn_time, mob_delayspawn, md->id, 0);
...
static int32 mob_ai_sub_hard_activesearch(struct block_list *bl, va_list ap) {
	...
	if (((*target) == nullptr || !check_distance_bl(md, *target, dist)) &&
...
static int32 mob_ai_sub_hard_changechase(struct block_list *bl, va_list ap) {
	...
	// If can't seek yet, not an enemy, or you can't attack it, skip.
	if ((*target) == bl || battle_check_target(md, bl, BCT_ENEMY) <= 0 || !status_check_skilluse(md, bl, 0, 0)) {
		return 0;
	}
	...
	if (mob_can_reach(md, bl, battle_config.loot_range) &&
		((*target) == nullptr || (battle_config.monster_loot_search_type && md->target_id > bl->id) ||
		 (!battle_config.monster_loot_search_type &&
		  !check_distance_bl(md, *target, dist)) // New target closer than previous one.
		 )) {
		(*target) = bl;
		md->target_id = bl->id;
	}
...
int32 mob_randomwalk(mob_data *md, t_tick tick) {
	...
	if (DIFF_TICK(md->next_walktime, tick) > 0 || status_has_mode(&md->status, MD_NORANDOMWALK) || !unit_can_move(md) ||
...
int32 mob_warpchase(mob_data *md, struct block_list *target) {
	...
	if (md->ud.walktimer != INVALID_TIMER && map_getcell(md->m, md->ud.to_x, md->ud.to_y, CELL_CHKNPC)) {
	...
	map_foreachinallrange(mob_warpchase_sub, md, md->db->range2, type, target, &warp, &distance);
...
static bool mob_ai_sub_hard(mob_data *md, t_tick tick) {
	...
	if (md->target_id) { // Check validity of current target. [Skotlex]
		...
		if (!tbl || tbl->m != md->m || (md->ud.attacktimer == INVALID_TIMER && !status_check_skilluse(md, tbl, 0, 0)) ||
		...
		if (tbl != nullptr && tbl->type != BL_ITEM && md->ud.walktimer == INVALID_TIMER &&
			...
			if (!battle_check_range(md, tbl, md->status.rhw.range)) {
				// If target is no longer visible, target is dropped and the monster waits for next iteration to
				// continue
		...
		if (md->attacked_id == md->target_id) { // Rude attacked check.
			...
			if (md->m != abl->m || abl->prev == nullptr ||
				...
				|| (battle_config.mob_ai & 0x2 &&
					!status_check_skilluse(md, abl, 0, 0)) // Cannot normal attack back to Attacker
				||
...
/**
 * Timer that triggers after walk delay when a berserk-state monster was attacked by a normal attack from within [attack
 * range+1] Same as mob_attacked, but sets a special normal attacked ID to trigger a target change in attack state
 * @param tid: Timer ID
 * @param tick: Current tick
 * @param id: ID of attacked monster
 * @param data: ID of attacker
 ...
 *------------------------------------------*/
static std::shared_ptr<s_item_drop> mob_setdropitem(const std::shared_ptr<s_mob_drop> &mobdrop,
													int32 qty,
													uint16 mob_id) {
 ...
 *------------------------------------------*/
static void mob_item_drop(mob_data *md,
...
int32 mob_dead(mob_data *md, struct block_list *src, int32 type) {
	...
	if (!(type & 2) && // No exp
		...
		for (i = 0; i < md->dmglog.size(); i++) {
			...
			if ((base_exp > 0 || job_exp > 0) && entry.flag == MDLF_HOMUN && homkillonly &&
	...
	if (!(type & 1) && !map_getmapflag(m, MF_NOMOBLOOT) && !md->state.rebirth &&
...
#ifdef RENEWAL_DROP
		drop_modifier = pc_level_penalty_mod(first_sd != nullptr    ? first_sd
		...
		for (const std::shared_ptr<s_mob_drop> &entry : md->db->dropitem) {
			...
			mob_item_drop(md,
						  ...
						  ditem,
						  0,
						  battle_config.autoloot_adjust ? drop_rate : entry->rate,
		...
		if (sd) {
			...
			if ((sd->mission_mobid == md->mob_id) ||
				(mission_mdb != nullptr && ((battle_config.taekwon_mission_mobname == 1 &&
											 util::vector_exists(status_get_race2(md), RC2_GOBLIN) &&
			...
			if (achievement_db.mobexists(md->mob_id)) {
				if (battle_config.achievement_mob_share > 0 && sd->status.party_id > 0) {
					map_foreachinallrange(
						achievement_update_objective_sub, md, AREA_SIZE, BL_PC, sd->status.party_id, md->mob_id);
				}
				else {
	...
	if (battle_config.mvp_tomb_enabled && md->spawn->state.boss && map_getmapflag(md->m, MF_NOTOMB) != 1) {
		mvptomb_create(
			md,
			mvp_sd != nullptr ? mvp_sd->status.name : (first_sd != nullptr ? first_sd->status.name : nullptr),
			time(nullptr));
	}
...
int32 mob_summonslave(mob_data *md2, int32 *value, int32 amount, uint16 skill_id) {
	...
	for (; k < amount; k++) {
		...
		if (skill_id != NPC_DEATHSUMMON &&
			map_search_freecell(md2, 0, &x, &y, MOB_SLAVEDISTANCE, MOB_SLAVEDISTANCE, 0)) {
			data.x = x;
			data.y = y;
		}
		else {
	...
	for (n = 0; n < ms.size(); i++, n++) {
		...
		if (ms[i]->cond1 == event) {
			...
			switch (ms[i]->cond1) {
				...
				case MSC_ALCHEMIST:
					flag =
						(md->special_state.ai != AI_NONE && md->trickcasting == 0 && md->status.hp < md->status.max_hp);
					break;
		...
		if (skill_get_casttype(ms[i]->skill_id) == CAST_GROUND) { // Ground skill.
			...
			switch (ms[i]->target) {
				case MST_RANDOM: // Pick a random enemy within skill range.
					bl = battle_getenemy(
						md, DEFAULT_ENEMY_TYPE(md), skill_get_range2(md, ms[i]->skill_id, ms[i]->skill_lv, true));
					break;
				case MST_TARGET:
				case MST_AROUND5:
				case MST_AROUND6:
			...
			if (ms[i]->target >= MST_AROUND5) {
				j = ms[i]->target >= MST_AROUND1 ? (ms[i]->target - MST_AROUND1) + 1
...
bool MobDatabase::parseDropNode(std::string nodeName,
								const ryml::NodeRef &node,
								uint8 max,
								std::vector<std::shared_ptr<s_mob_drop>> &drops) {
...
uint64 MobDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "Title")) {
		...
		if (title.size() > NAME_LENGTH) {
			this->invalidWarning(node["Title"],
								 "Title \"%s\" exceeds maximum of %d characters, capping...\n",
								 title.c_str(),
								 NAME_LENGTH - 1);
		}
	...
	for (uint8 fi = 0; fi < ARRAYLENGTH(mob_db_name); ++fi) {
		...
		if (SQL_ERROR ==
			Sql_Query(
				...
				"hidden`,"
				"`racegroup_bio5_swordman_thief`,`racegroup_bio5_acolyte_merchant`,`racegroup_bio5_mage_archer`,`"
				...
				"illusion_vampire`,"
				"`racegroup_malangdo`,`racegroup_ep172alpha`,`racegroup_ep172beta`,`racegroup_ep172bath`,`racegroup_"
				"illusion_turtle`,`racegroup_rachel_sanctuary`,`racegroup_illusion_luanda`,`racegroup_illusion_frozen`,"
				"`racegroup_illusion_moonlight`,`racegroup_ep16_def`,`racegroup_edda_arunafeltz`,`racegroup_lasagna`,`"
				"racegroup_glast_heim_abyss`,"
...
static void mob_drop_ratio_adjust(void) {
	for (auto &pair : mob_db) {
		...
		for (auto it = mob->mvpitem.begin(); it != mob->mvpitem.end();) {
			...
			int32 rate = mob_drop_adjust(
				entry->rate, rate_adjust, battle_config.item_drop_mvp_min, battle_config.item_drop_mvp_max);
...
static int32 mob_reload_sub_npc(struct npc_data *nd, va_list args) {
	...
	if (mobdb_checkid(nd->class_)) {
		...
		if (nd->prev) { // If they are spawned right now
			unit_refresh(
				nd); // Respawn all NPCs on client side so that they are displayed correctly(if their view id changed)
		}
	}
