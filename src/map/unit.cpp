...
bool unit_update_chase(block_list &bl, t_tick tick, bool fullcheck) {
	...
	if (tbl != nullptr && tbl->type != BL_ITEM && tbl->m == bl.m && ud->walkpath.path_pos > 0 &&
		check_distance_bl(&bl, tbl, ud->chaserange)) {
		...
		if (stop_flag != USW_NONE) {
			// We need to make sure the walkpath is cleared here so a monster doesn't continue walking in case it
			// unlocked its target
...
bool unit_walktoxy_nextcell(block_list &bl, bool sendMove, t_tick tick) {
	...
	// Monsters first check for a chase skill and if they didn't use one if their target is in range each cell after
	// checking for a chase skill
	if (bl.type == BL_MOB) {
		mob_data &md = reinterpret_cast<mob_data &>(bl);
		...
		if (!ud->state.force_walk && DIFF_TICK(tick, md.last_skillcheck) > MOB_SKILL_INTERVAL - md.status.speed / 2 &&
		...
		case BL_PC:
			...
			for (size_t i = 0; i < sd->areanpc.size(); i++) {
				...
				if (!nd || nd->subtype != NPCTYPE_SCRIPT ||
					!(nd->m == bl->m && bl->x >= nd->x - nd->u.scr.xs && bl->x <= nd->x + nd->u.scr.xs &&
					  bl->y >= nd->y - nd->u.scr.ys && bl->y <= nd->y + nd->u.scr.ys)) {
...
void unit_data::update_pos(t_tick tick) {
	...
	if (cell_percent > 0.0 && cell_percent < 1.0) {
		...
		this->pos.sx =
			static_cast<uint8>(24.0 + dirx[this->walkpath.path[this->walkpath.path_pos]] * 16.0 * cell_percent);
		this->pos.sy =
			static_cast<uint8>(24.0 + diry[this->walkpath.path[this->walkpath.path_pos]] * 16.0 * cell_percent);
		...
		case BL_PC:
			switch (event) {
				...
				case DELAY_EVENT_PARRY:
					// Officially for players it just remembers the last attack time here and applies the delays during
					// the comparison But we pre-calculate the delays instead and store them in attackabletime and
					// canact_tick
					...
					act_delay = status_get_amotion(&bl) +
								(pc_maxaspd(reinterpret_cast<map_session_data *>(&bl)) / AMOTION_DIVIDER_PC);
					break;
			}
			break;
		case BL_MOB:
...
int32 unit_skilluse_id2(struct block_list *src,
	...
	switch (skill_id) {
		...
		case MO_EXTREMITYFIST:
			if (sc && sc->getSCE(SC_COMBO) &&
				(sc->getSCE(SC_COMBO)->val1 == MO_COMBOFINISH || sc->getSCE(SC_COMBO)->val1 == CH_CHAINCRUSH)) {
				casttime = -1;
			}
			combo = 1;
			break;
	...
	clif_skillcasting(
		*src, target, 0, 0, skill_id, skill_lv, static_cast<e_element>(skill_get_ele(skill_id, skill_lv)), casttime);
...
int32 unit_skilluse_pos2(struct block_list *src,
	...
	clif_skillcasting(*src,
					  ...
					  skill_id,
					  skill_lv,
					  static_cast<e_element>(skill_get_ele(skill_id, skill_lv)),
					  casttime);
		...
		if (md) {
			if (status_has_mode(sstatus, MD_ASSIST) && DIFF_TICK(tick, md->last_linktime) >= MIN_MOBLINKTIME) {
				...
				map_foreachinshootrange(
					mob_linksearch, src, battle_config.assist_range, BL_MOB, md->mob_id, target->id, tick);
			}
		}
	...
	switch (bl->type) {
		case BL_PC: {
			...
			if (battle_config.friend_auto_add) {
				...
				map_foreachpc(
					clif_friendslist_toggle_sub, sd->status.account_id, sd->status.char_id, static_cast<int32>(false));
			}
