	...
	if (this->nodeExists(node, "Aliases")) {
		...
		for (const auto& subNode : aliasesNode) {
			...
			if (parent_cmd != nullptr) {
				this->invalidWarning(
					subNode,
					"\"%s\" cannot be used as alias for \"%s\", since a command with this name exists.\n",
					alias.c_str(),
					command.c_str());
				return 0;
			}
...
ACMD_FUNC(jump) {
	...
	if ((x || y) &&
		map_getcell(sd->m, x, y, CELL_CHKNOPASS)) { // This is to prevent the pc_setpos call from printing an error.
		clif_displaymessage(fd, msg_txt(sd, 2)); // Invalid coordinates, using random target cell.
...
ACMD_FUNC(hide) {
	...
	if (pc_isinvisible(sd)) {
		...
		if (!battle_config.pk_mode && map_getmapflag(sd->m, MF_PVP) &&
			!map_getmapflag(sd->m, MF_PVP_NOCALCRANK)) { // register the player for ranking calculations
...
ACMD_FUNC(guildlevelup) {
	...
	if (level > 0 && (level > MAX_GUILDLEVEL ||
					  added_level > ((int16)MAX_GUILDLEVEL - guild_info->guild.guild_lv))) { // fix positive overflow
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter)) {
		if (sd->status.account_id != pl_sd->status.account_id && pl_sd->status.guild_id == g->guild.guild_id) {
			if (pc_get_group_level(pl_sd) > pc_get_group_level(sd) ||
				(pl_sd->m == sd->m && pl_sd->x == sd->x && pl_sd->y == sd->y)) {
...
ACMD_FUNC(displayskillcast) {
	...
	if (target_type == 1) {
		...
		clif_skillcasting(
			*sd, sd, 0, 0, skill_id, skill_lv, static_cast<e_element>(skill_get_ele(skill_id, skill_lv)), cast_time);
	}
	...
	if (!message || !*message || (sscanf(message, "%49s %6hd", flag_name, &flag) < 1)) {
		...
		for (i = MF_MIN; i < MF_MAX; i++) {
			...
			if (map_getmapflag_name(static_cast<e_mapflag>(i), flag_name) &&
				map_getmapflag_sub(sd->m, static_cast<e_mapflag>(i), &args)) {
...
ACMD_FUNC(vip) {
	...
	if (pl_sd->vip.time <= now) {
		...
		sprintf(
			...
			hour,
			minute,
			second); // Your VIP status is valid for %d years, %d months, %d days, %d hours, %d minutes and %d seconds.
		clif_displaymessage(pl_sd->fd, atcmd_output);
		...
		if (pl_sd != sd) {
			sprintf(atcmd_output,
					...
					hour,
					minute,
					second); // Player '%s' is now VIP for %d years, %d months, %d days, %d hours, %d minutes and %d
							 // seconds.
			clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(macrochecker) {
	...
	int32 count =
		map_foreachinmap(atcommand_macrochecker_sub, mapid, BL_PC, sd->status.account_id, pc_get_group_level(sd));
...
void atcommand_doload(void) {
	...
	for (AtCommandInfo* command_info = static_cast<AtCommandInfo*>(dbi_first(atcommand_iter));
		 dbi_exists(atcommand_iter);
		 command_info = static_cast<AtCommandInfo*>(dbi_next(atcommand_iter))) {
		...
		if (info == nullptr) {
			ShowError("atcommand_doload: No entry for atcommand \"%s\" found in \"%s\"\n",
					  ...
					  atcommand_alias_db.getDefaultLocation().c_str());
			continue;
		}
