...
int8 buyingstore_create(map_session_data *sd,
	...
	if (pc_getpercentweight(*sd, weight) >=
		battle_config.major_overweight_rate) { // not able to carry all wanted items without getting overweight (90%)
		sd->buyingstore.slots = 0;
...
void buyingstore_open(map_session_data *sd, uint32 account_id) {
	...
	if (!searchstore_queryremote(*sd, account_id) &&
		(sd->m != pl_sd->m || !check_distance_bl(sd, pl_sd, AREA_SIZE))) { // out of view range
		return;
	}
